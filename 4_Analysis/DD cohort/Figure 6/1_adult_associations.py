...
OUTPUT_STATS="/paht/to/output/statistics/file.csv" # These are the statistics presented in Table S5E
OUTPUT_HEATMAP="/path/to/output/heatmap/figure.pdf" # This plot will be the heatmap in Fig 6D left

# Load data
df=pd.read_csv(TABS1A)
df=df[(df['16p12.1 deletion']=='Carrier') & (~df.Relationship.isin(['Proband', 'Sibling', 'Cousin']))]
...
phenos=['Depression (Questionnaire)', 'Anxiety (Questionnaire)', 'Sleep trouble (Questionnaire)', 'Psychosis (Questionnaire)', 'Addiction (Questionnaire)', 'Mood lability (Questionnaire)']
...
for v in vars:
	for p in phenos:
		...
		# Save stats
		stat_lst.append([p.split(' (')[0], v, 'Two tailed t-test', np.mean(x), np.std(x), len(x), np.mean(y), np.std(y), len(y), cohens_d(x, y), res.statistic, res.pvalue])
...
pheno_order=['Depression', 'Anxiety', 'Sleep trouble', 'Psychosis', 'Addiction', 'Mood lability']
