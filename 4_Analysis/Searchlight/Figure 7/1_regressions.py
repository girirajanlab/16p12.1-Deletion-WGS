...
#	(B) Phenotype ~ All coding SNVs + Genes del. + Genes dup. + Sex + SCZ PRS
#	(C) Phenotype ~ All coding SNVs (LF) + Genes del. (LF) + Genes dup. (LF) + Sex + SCZ PRS
...
vars=['All coding SNVs', 'Genes del.', 'Genes dup.',
		'SCZ PRS', 
		'All coding SNVs (LF)', 'Genes del. (LF)', 'Genes dup. (LF)']
...
for cohort in cohorts:
	...
	df=df[(~df[quant_phenos].isnull().all(axis=1)) & (~df.Sex.isnull()) & (~df['SCZ PRS'].isnull()) & (~df['All coding SNVs'].isnull()) & (~df['Genes del.'].isnull())]
	...
	model_inputs={'B':['All coding SNVs', 'Genes del.', 'Genes dup.', 'SCZ PRS', 'Sex'],
					'C':['All coding SNVs (LF)', 'Genes del. (LF)', 'Genes dup. (LF)', 'SCZ PRS', 'Sex']}
...
outdf2=outdf.copy()
outdf2['Model']=outdf2.Model.map({'B':'Phenotype ~ Sex + All coding SNVs + Genes del. + Genes dup. + SCZ PRS',
									'C':'Phenotype ~ Sex + All coding SNVs (LF) + Genes del. (LF) + Genes dup. (LF) + SCZ PRS'})
...
# Organize data
outdf['Variable']=pd.Categorical(outdf.Variable, ['All coding SNVs', 'Genes del.', 'Genes dup.', 'All coding SNVs (LF)', 'Genes del. (LF)', 'Genes dup. (LF)', 'SCZ PRS'])
...
for mod in ['B', 'C']:
	for pheno in quant_phenos:
		...
		palette={'16p11.2 deletion':'#5D3E98', '16p11.2 duplication':'#0C808D'}
		...
		# Change the x labels
		locs=[0.5, 4.5, 8.5, 12.5]
		if mod=='B':
			plt.yticks(locs, reversed(['All coding SNVs', 'Genes del.', 'Genes dup.', 'SCZ PRS']))
		if mod=='C':
			plt.yticks(locs, reversed(['All coding SNVs (LF)', 'Genes del. (LF)', 'Genes dup. (LF)', 'SCZ PRS']))
