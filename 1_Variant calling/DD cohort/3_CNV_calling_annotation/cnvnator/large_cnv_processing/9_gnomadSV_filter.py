...
lookup_path='path/to/lookup/files' # These are the files from script 8_gnomadSV_annotation.sh
cnv_path='path/to/cnv/files' # These are the files from script 6_frequency_filter.py
...
for cnv_type in ['cnv', 'pathogenic_cnv']:
	for cn in ['dels', 'dups']:
		lookup=pd.read_csv(f'{lookup_path}/{cnv_type}_gnomadSV_anno_{cn}.bed', sep='\t')
		df=pd.read_csv(f'{cnv_path}/{cnv_type}_frequency_filter_{cn}.bed', sep='\t')
		...
		# Save
		df.to_csv(f'{output_path}/{cnv_type}_gnomadSV_filter_{cn}.bed', index = False, sep = '\t')
