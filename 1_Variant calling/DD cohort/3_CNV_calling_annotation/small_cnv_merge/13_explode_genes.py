...
input_file='path/to/cnv/file/inherited_cnvs.bed' # This is the output from script 12_inhertiance_annotation.py
...
gene_df = pd.DataFrame(columns = ['Sample', 'Gene_ID', 'Gene_Symbol', 'Type'])
for idx, row in calls.iterrows():
	...
	if len(gene_ids) != len(gene_symbols):
		...
		for key in id_dict.keys():
			out_lines.append([sample, ' '.join(id_dict[key]), key, type, inh])
		...
		for i in range(len(gene_ids2)):
			out_lines.append([sample, gene_ids2[i], gene_symbols[i], type, inh])

	# Add to dataframe
	gene_df = pd.concat([gene_df, pd.DataFrame(out_lines, columns = ['Sample', 'Gene_ID', 'Gene_Symbol', 'Type', 'Inheritance'])], axis = 0, ignore_index = True)
