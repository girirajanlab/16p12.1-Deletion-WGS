...
LOPLINK=/path/to/liftOverPlink.py # This script can be downloaded from https://github.com/sritchie73/liftOverPlink/blob/master/liftOverPlink.py
...
# Remove SNPs in the 16p12.1 deletion region
/data5/software/plink --bfile $OUTPUT_DIR/DEDUP --make-bed --keep $OUTPUT_DIR/HETEROZYGOSITY.valid.sample --out $OUTPUT_DIR/EXCLUDE_16P --extract $OUTPUT_DIR/SAMPLE_QC.snplist --write-snplist --exclude-snps chr16:21783909:G:A-chr16:22701658:G:T

# Recode PLINK bfiles to make PED/MAP format
$PLINK --bfile $OUTPUT_DIR/EXCLUDE_16P --recode tab --out $OUTPUT_DIR/RECODE
