...
genes.gene_id=genes.gene_id.str.split('.', expand=True)[0]
...
exons.gene_id=exons.gene_id.str.split('.', expand=True)[0]
...
calls['gene_ids']=calls.exon_overlap.apply(lambda row: ' '.join(row))

# Remove extra columns
cols = ['Chr', 'Start', 'End', 'Type', 'Zygosity', 'Length', 'NumSNP', 'Sample', 'StartSNP', 'EndSNP', 'Merge',
	'Control_num', 'Control_freq', 'SPARK_num', 'SPARK_freq', 'variant_id', 'Pathogenic_Name', 'gene_ids', 'gene_symbols']
calls=calls[cols]
