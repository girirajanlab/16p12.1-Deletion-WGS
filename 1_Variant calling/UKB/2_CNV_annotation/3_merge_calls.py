...
SNP_Pos="/path/to/SPARK_SNP_Positions.list" # Use the SNP position file generated in script 1_run_PennCNV.sh
pcnv_output="/path/to/PennCNV/output/directory" # Use the OUTPUT_DIR from script 1_run_PennCNV.sh
batch="batch_number" # i.e. 1, 2, etc.
output_dir="/path/to/output/directory"

# Load files
filter = pd.read_csv(QC_file, sep = '\t')
filter['File'] = filter.File.str.split(' ', expand = True)[2]
passed = filter[(filter.Pass_auto == True) & (filter.Pass_chrX==True)]['File'].to_list()
...
calls['File'] = calls.File.str.split(' ', expand = True)[2]
...
calls['Sample'] = calls.File.str.split('/', expand = True)[-1].str.split('_', expand = True)[0]
...
final_out.to_csv(f'{output_dir}/{batch}_merged.txt', sep = '\t', index = False)
